...
export default function WQEDCOMC() {
  ...
  const WQFilterEntity = "WQEDCOMCfilters"
  ...
  const load = async () => {
    ...
    const [ {result}  , {result: result1} , admin ] = await Promise.all([ await request.read('billingcolorWQEDCOMC', 1), await request.list(entity+"-columns", {id: current.EMPID}), await request.list("admin")]);

    // const { result = [] } = await request.read('billingcolorWQEDCOMC', 1);
    if (result.length === 0) {
      await request.create('billingcolorWQEDCOMC', billingColorData);
      setLoaded(true)
      return setDataTableColorList(defaultColors)
    }

    // let usersList = admin.result.filter(res => res.ManagementAccess == 0 || res.ManagementAccess == null ).map((user) => ({EMPID: user.EMPID, name: user.Nickname , last: user.Last, text:  user.Nickname  , status: 'success'}))
  ...
  const entity = "WQEDCOMC";
  const loggerEntity = "WQEDCOMCLogger";
  ...
  const onNotesAction = (id, status) => {
    ...
    dispatch(crud.create(loggerEntity, { IDWQEDCOMC: id, UserName: current.name, BatchID: d['Batch Id'], Status: status, DateTime: currentDate }))
  }
  ...
  const handleSaveColor = (EMPID, data) => {
    request.update("billingcolorWQEDCOMC", EMPID, data);
  }
  ...
  const onCopied = (id, batchID, text) => {
    dispatch(crud.create(loggerEntity, { IDWQEDCOMC: id, UserName: current.name, Color: "", Status: "Copy Batch ID", DateTime: currentDate, 'BatchID': batchID , CopiedText: text}))
  }
...
// Select Modal
      ...
      >
        {
          ...
          <Select mode="multiple">
            ...
            <Option value="Wrong Other">Wrong Other</Option>
  ...
  const panelTitle = "WQ MC EDCO Document Integrity";
  const dataTableTitle = "WQ MC EDCO Document Integrity";
  const progressEntity = "WQEDCOMCprogress";
  const workEntity = "WQEDCOMCWork";
  const showProcessFilters = true;


  const onWorkSaved = async (amount) => {
    const response = await request.list("WQEDCOMCWork");
  ...
  const updateTime = async (id, value, cb, ent, entry, row) => {
    ...
    if(ent == 'Stop') {
      ...
      await request.create('WQEDCOMC-updatetime',  value)
    } else {
      value.Status = ent
    }    
    ...
    if(typeof id == 'number') {
      ...
      dispatch(crud.create(loggerEntity, { IDWQEDCOMC: id, UserName: current.name, BatchID: item['Batch Id'], Status: value.Status, DateTime: currentDate }))
    }  else if (ent != 'Stop'){
