...
const endpoints = methods.crudController("WQEDCOMFFilters");
var sql = require("mssql");

const Model = "WQEDCOMFFilters";
const WQModel = 'WQEDCOMF'
delete endpoints["list"];

const EDCOMFFilterModel = 'WQEDCOMFFilters'
...
setInterval(async() => {
  ...
  if (hours == 23 && minutes == 50) {
    const  {recordset: result2} = await sql.query(`select * from  ${EDCOMFFilterModel}  `)

    if(result2.length<5) {
      for(let i=0 ; i< 5- result2.length ; i++) {
        await sql.query(`insert into  ${EDCOMFFilterModel} (EMPID) values  (NULL)  `)
      }
    }
  }
}, 50000)
