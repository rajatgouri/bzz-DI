...
const endpoints = methods.crudController("WQEDCOMC");
...
const Model = "WQEDCOMC";
const ColumnModel = "WQEDCOMCColumns"
const Filter_Model = "WQEDCOMCFilters"
            ...
            if(filter[key]) {
                ...
                switch (key) {
                    ...
                    case "Document Description": {
                        ...
                        valueQuery = values.filter(v => v != "" ).map(v => ( "'" + v.replace(/'/g, "''")  + "'"))
...
//     //         await sql.query(`Select count(*) as count  from ${Model} where Status NOT IN ('Review')`),
...
//     //     const [{recordset: chargesProcessedCount}, {recordset: chargesReviewCount}, {recordset: chargesReview}, {recordset: notToReview}, {recordset: total}]  = await Promise.all([
//     //         await sql.query(`Select count(*) as count from ${Model} where Status IN ('Done') and UserAssigned IN ('${First}')`),
//     //         await sql.query(`Select count(*) as count from ${Model} where Status NOT IN ('Done') and UserAssigned IN ('${First}')`),
//     //         await sql.query(`Select count(*) as count from ${Model} where Status NOT IN ('Review') and UserAssigned IN ('${First}')`),
//     //         await sql.query(`Select * from ${Model} where Status NOT IN ('Done') and UserAssigned IN ('${First}')`),
//     //         await sql.query(`Select count(*) as count from ${Model} where UserAssigned IN ('${First}')`),
//     //     ])
...
const getObject = (obj) => {
    return  obj.map((o) => {
        ...
        // return  [
            // (o['Correct'] ? o['Correct'] .toString() : ''),
            // (o['Error'] ? o['Error'] .toString() : ''),
            // (o['Error Type'] ? o['Error Type'] .toString() : ''),
            // (o['Error Tracking'] ? o['Error Tracking'] .toString() : ''),
            // (o['Comments'] ? o['Comments'] .toString() : ''),
            // (o['Batch Id'] ? o['Batch Id'] .toString() : ''),
            // (o['Med Rec'] ? o['Med Rec'] .toString() : ''),
            // (o['Account Number'] ? o['Account Number'] .toString() : ''),
            // (o['Patient Name'] ? o['Patient Name'] .toString() : ''),
            // ( o['Capture Location'] ? o['Capture Location'] .toString() : ''),
            // (o['Batch Capture Class'] ? o['Batch Capture Class'] .toString() : ''),
            // (o['Batch Type'] ? o['Batch Type'] .toString() : ''),
            // ( o['Edco Document Type'] ? o['Edco Document Type'] .toString() : ''),
            // (o['Document Type'] ? o['Document Type'] .toString() : ''),
            // ( o['Document Description'] ? o['Document Description'] .toString() : ''),
            // ( o['Document Id'] ? o['Document Id'] .toString() : ''),
            //  (o['Release Date'] ? o['Release Date'].toISOString().replace('.000Z', '') .toString() : ''),
            // (o['Order Number'] ? o['Order Number'] .toString() : ''),
            // (o['Order Date'] ? new Date(o['Order Date']).toISOString().split('.')[0].split('T')[0] .toString() : ''),
            // ( o['Chart Correction'] ? o['Chart Correction'] .toString() : ''),
            // ( o['Document Pages'] ? o['Document Pages'] .toString() : ''),
            // ( o['UserAssigned'] ? o['UserAssigned'] .toString() : ''),
            // (o['Status'] ? o['Status'] .toString() : ''),
            // (o['User'] ? o['User'] .toString() : ''),
            // ( o['UploadDateTime'] ? new Date(o['UploadDateTime']).toISOString().split('.')[0].split('T').join(' ')  : ''),
            // (o['StartTimeStamp'] ? new Date(o['StartTimeStamp']).toISOString().split('.')[0].split('T').join(' ')  : ''),
            // ( o['FinishTimeStamp'] ? new Date(o['FinishTimeStamp'] ).toISOString().split('.')[0].split('T').join(' ') : ''),
            // (o['Duration'] ? o['Duration'] .toString() : ''),
            // (o['Archive'] ? o['Archive'].toString() : ''),
 ...
 endpoints.exports = async (req, res) => {
    try {
        ...
        let file = `WQEDCOMC_SmartApp_${utilController.getDateTime().toString().replace(/-/g, '_').replace(/:/g, '_').split('.')[0]}.xlsx`
    ...
    try {
        ...
        valuesQuery = valuesQuery.slice(0, -1);
            ...
            if (status.indexOf('All') > -1) {
                r  = `update ${Model} set Error='No' , Correct='Yes' where [Batch Id]  IN ('${mrn}')   and (Status IN ('QA Review', '') or Status IS NUll)  and UserAssigned IS NOT NULL  `
                await sql.query(r);
                q = `update ${Model} set ${valuesQuery} where [Batch Id]  IN ('${mrn}')   and (Status IN ('QA Review', '') or Status IS NUll)  and UserAssigned IS NOT NULL  `
                await sql.query(q);
